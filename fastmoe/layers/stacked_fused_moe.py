...
def invoke_fused_moe_kernel(A: torch.Tensor, B: torch.Tensor, B_ind: torch.Tensor, C: torch.Tensor,
                            ...
                            mul_routed_weight: bool, top_k: int, config: dict, N, K, w2_off):
...
def stack_fused_moe(
    ...
    """
    This function computes a Mixture of Experts (MoE) layer using the experts weights, and top-k gating mechanism.
    
    Parameters:
    - hidden_states (torch.Tensor): The input tensor to the MoE layer.
    - w1 (torch.Tensor): The expert weights memory pool on gpu. each page: [W1, W3, W2]
    - indicies (torch.Tensor): The indices of the experts weights in the memory pool.
    ...
    invoke_fused_moe_kernel(hidden_states, w1.view(w1.shape[0], -1, H), indicies, intermediate_cache1,
