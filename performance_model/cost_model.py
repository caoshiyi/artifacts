...
class CostModelConfig:
    ...
    gpu_flops: float = 242 * T
    cpu_flops: float = 13 * T

    tp_size: int = 1


def solve_lp(config, bls, gbs, verbose=1, stage="decode", gattn=False):
    ...
    prob += gpu_home_d == w_other + wi * l * wg + 2 * 2 * h1 * bls * tp_size + 4 * (s + n) * h_kv * bls * l * (cg + cc * gattn)
    ...
    # prob += gpu_home_p == w_other + wi * l * wg + 4 * s * h_kv * bls * cg + tp_size * (2 * h1 * gbs * 2 * s + h1 * bls * 2 * s) + 4 * s * h_kv * gbs * 2 * cc
    prob += gpu_home_p == w_other + wi * l * wg + 4 * s * h_kv * bls * cg + tp_size * (2 * 2 * h1 * gbs * 2 * s) + 4 * s * h_kv * gbs * 2 * cc

    # for the fused moe kernel
    # if stage == "decode":
    #     prob += inter == gbs * topk * h1 + gbs * topk * 3 * h2
    # elif stage == "prefill":
    prob += inter == 2 * (gbs * topk * h1 * tp_size * s + gbs * topk * 3 * h2 * s) * 1.2
    ...
    # elif stage == "prefill":
    # fix this for tp: only one GPU offload hidden states
    prob += cpu_home == wi * l * wc + 4 * (s + n) * h_kv * bls * l * cc + 2 * h1 * bls * s * tp_size
    ...
    if verbose:
        ...
        print(f"non-experts weights size: {(w_other) / GB:.4f} GB")
...
def get_nb_ub(config, gbs, solve_lp, gattn=False):
    nb = 1
    while True:
        status, _, _, _, _ = solve_lp(config, gbs * nb, gbs, verbose=0, gattn=gattn)
        if status == -1: break
        nb *= 2
    ...
    while left < right:
        mid = (left + right) // 2
        status, _, _, _, _ = solve_lp(config, gbs * mid, gbs, verbose=0, gattn=gattn)
        if status == 1:
            left = mid + 1
        elif status == -1:
            right = mid
...
def solve(config, args, gattn=False):
    best_policy = None
    max_throughput = 0
    gbs = 1
    while True:
        ...
        if args["num_gb"] is not None:
            ...
            for nb in range(1, nb_ub + 1):
                _, policy, (throughput, t_tot), _, _ = solve_lp(config, gbs * nb, gbs, verbose=0, stage=args["stage"], gattn=gattn)
    ...
    if best_policy is not None:
        _, _, _, _, _ = solve_lp(config, best_policy.ubs * best_policy.n_ub,
                           best_policy.ubs, verbose=True, stage=args["stage"], gattn=gattn)
    return best_policy, max_throughput

def get_throughput(model_config, cpu_mem, gpu_mem, cpu_bw, gpu_bw, cpu_flops, gpu_flops, cpu_gpu_bdw, gattn=False):
    config = CostModelConfig()
    config.s = 512
    config.n = 128
    config.gmem = gpu_mem * GB
    config.cmem = cpu_mem * GB
    config.ctog_bdw = cpu_gpu_bdw * GB
    config.g_bdw = gpu_bw * GB
    config.c_bdw = cpu_bw * GB
    config.gpu_flops = gpu_flops * T
    config.cpu_flops = cpu_flops * T
    config.tp_size = 1

    config.l=model_config.num_hidden_layers
    config.h1=model_config.hidden_size
    config.h2=model_config.intermediate_size
    config.nh=model_config.num_attention_heads
    config.nkvh=model_config.num_key_value_heads
    config.n_experts=model_config.num_local_experts
    config.topk=model_config.topk
    ...
    best_policy, max_throughput = solve(config, args, gattn=gattn)
    print(best_policy)
    return max_throughput, best_policy

# num_cpu_node = 1
# cpu_mem = 200 * num_cpu_node
# cpu_bw = 100 * num_cpu_node
# cpu_flops = 1.6 * num_cpu_node

# num_gpus = 2
# gpu_mem = 80 * num_gpus
# gpu_bw = 2000 * num_gpus
# gpu_flops = 300 * num_gpus

# cpu_gpu_bdw = 500
# config = ModelConfig("mistralai/Mixtral-8x7B-Instruct-v0.1")
# gpu_attn_throughput, gpolicy = get_throughput(config, cpu_mem, gpu_mem, cpu_bw, gpu_bw, cpu_flops, gpu_flops, cpu_gpu_bdw, True)
# cpu_attn_throughput, cpolicy = get_throughput(config, cpu_mem, gpu_mem, cpu_bw, gpu_bw, cpu_flops, gpu_flops, cpu_gpu_bdw, False)
...
#     best_policy, max_throughput = solve(moe_config, vars(args))
#     print(best_policy)
